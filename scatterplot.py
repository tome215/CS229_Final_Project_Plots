...
for i in range(0, num_files):
	...
	for k in site_count_dict.keys():
		# plt.scatter([i], [k], color=(1, 0, 0, math.tanh(site_count_dict[k])))
		# if (i, k) in countsDict.keys():
		countsDict[(i, math.log(k+1))] += site_count_dict[k]
		# else:
			# countsDict[(i, math.log(k+1))] = site_count_dict[k]
	...
	for k in site_count_dict.keys():
		total += math.log(k+1)*site_count_dict[k]
...
print("Scattering in progress...")
plt.scatter(
	list(map(lambda t: t[0], countsDict.keys())),
	[t[1] for t in countsDict.keys()],
	color=[(1, 0, 0, math.tanh(countsDict[t])) for t in countsDict.keys()]
)
plt.plot(avgArr, color=(0, 0, 1, 1))
